...
`rssr_`. The remainder of this document is in two main sections. If you're
...
5. A standard may be addressed by moving the item in the `rssr_standards.R`
   ...
   which refers to a particular standard at all locations which directly
...
6. Alternatively, any standards which you consider not applicable to your
   ...
   standard not applicable to your software. These `@rssrNA` tags should be
   ...
   standards can then just be moved into this block, with their `@rssrTODO`
   tags changed to `@rssrNA` 
7. Each time you run 
...
Running a second time will generate cleaner output from the `rssr` roclets
...
parse the various `@rssr` tags in all of these locations.
