...
documentation and triggering the `rssr` roclets will be referred to as "running
...
to help rectify any problems. `rssr` documentation must adhere to the following
...
block contained within a dedicated code chunk. `rssr` chunks within `.Rmd`
files will generally use `echo = FALSE` to prevent them appearing in the
...
2. Change your package's `DESCRIPTION` file to use the `rssr` roclets by adding
   `roclets = "rssr::rssr_roclet"`) to the `Roxygen` line, as demonstrated at
