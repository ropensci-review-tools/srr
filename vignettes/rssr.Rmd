...
function](https://ropenscilabs.github.io/rrr/reference/rssr_pkg_skeleton.html)
...
### 3.2 Locations of `rrr` documentation
...
Those lines illustrate the expected form. `@rssr` tags should be within
a single block contained within a dedicated code chunk. `@rssr` chunks within
...
While `@rssrTODO` and `@rssr` tags may be placed anywhere within a package,
`@rssrNA` tags used to denote non-applicable standards must be placed within
...
1. Automatically download and insert lists of general and category-specific
   ...
   `R/` directory called `rssr-standards.R`, although any alternative name can
   also be passed to the function (or the file can be renamed after it has
   initially been created).
2. Change your package's `DESCRIPTION` file to use the `rrr` roclets by adding
   `roclets = "rrr::rssr_roclet"`) to the `Roxygen` line, as demonstrated at
...
3. Run [`roxygenise`](https://roxygen2.r-lib.org/reference/roxygenize.html) to
   ...
   a single list of `@rssrTODO` standards.
...
5. Begin addressing the standards by:
    ...
    - Modifying the `@rssrTODO` tag to `@rssr`
...
7. Update your documentation as frequently as you like or need, and use the
   ...
   `@rssrTODO` tags into either `@rssr` or `@rssrNA` tags.
