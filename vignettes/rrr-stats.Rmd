---
title: "Preparing Statistical Software with the rrr package"
author: 
  - "Mark Padgham"
date: "`r Sys.Date()`"
output: 
...
`rrr` stands for [**r**OpenSci](https://ropensci.org) **r**eview **r**oclets,
and is an R package containing roclets for general use in helping those
developing and reviewing packages submitted to rOpenSci. At present, the `rrr`
package only contains roclets and associated functions to help those developing
and reviewing packages submitted to rOpenSci's system for [Statistical Software
...
documentation and triggering the `rrr` roclets will be referred to as "running
...
The [`rrr_stats_pkg_skeleton()`
function](https://ropenscilabs.github.io/rrr/reference/rrr_stats_pkg_skeleton.html)
...
## 2. Enabling `rrr` roclets for a package
...
not need to add, import, or depend upon the `rrr` package anywhere else within
the `DESCRIPTION` file. See the `DESCRIPTION` file of the package skeleton for
an example.

## 3. `roxygen2` tags

The `rrr` packages recognises and uses the following three tags, each of which
may be inserted into [`roxygen2`](https://roxygen2.r-lib.org) documentation
blocks anywhere in your code. The tags are:

1. `@rrrstats` tags to indicate standards which have been addressed. These should
...
2. `@rrrstatsTODO` tags to indicate standards which have not yet been addressed.
   The [`rrr_stats_roxygen()`
   function](https://ropenscilabs.github.io/rrr/reference/rrr_stats_roxygen.html)
   ...
   a tag of `@rrrstatsTODO`. As you address each standard, you'll need to move it
   from that initial location to that point in your code where that standard is
   addressed, and change the tag from `@rrrstatsTODO` to `@rrrstats`. It may help to
   ...
   directory.
3. `@rrrstatsNA` tags to indicate standards which you deem not applicable to your
   ...
   also in the output of [`rrr_stats_roxygen()`
   function](https://ropenscilabs.github.io/rrr/reference/rrr_stats_roxygen.html).
...
The output illustrated above shows how the `rrr` package groups these three
tags together, first collecting the output of standards which have been
addressed (via `@rrrstats` tags), then showing output of non-applicable standards
(via `@rrrstatsNA` tags), and finally standards which are still to be addressed
(via `@rrrstatsTODO` tags).

### 3.1 Format of `rrr` documentation
...
to help rectify any problems. `rrr` documentation must adhere to the following
formatting requirements:

1. The tags must appear immediately after the
   [`roxygen2`](https://roxygen2.r-lib.org)-style comment symbols. This will
   almost always mean `#' @rrrstats` (but see details below, under *Locations of
   `rrrstats` documentation*).
2. The standards number(s) must be enclosed within curly braces, such as 
   `#' @rrrstats {G1.0}`. Multiple standards may be associated with a single tag by
   including them within the same single pair of curly braces, and separating
   each by a comma, such as `#' @rrrstats {G1.0, G1.1}`.
3. Explanatory text may be placed anywhere before or after curly braces
   enclosing standards, such that `#' @rrrstats some text {G1.0}` is entirely
   equivalent to `#' @rrrstats {G1.0} some text`.
...
5. Any standard may occur arbitrarily many times in any file(s) within a
   ...
   associated with one single kind of tag; thus should only be `@rrrstats` (a
   standard which has been addressed), *or* `@rrrstatsNA` (a standard which is not
   applicable), *or* `@rrrstatsTODO` (a standard which has not yet been addressed).
...
### 3.2 Locations of `rrrstats` documentation

`rrrstats` tags and accompanying text can be placed almost anywhere within a
...
```{r doc-ex-with-fn}
#' @rrrstats G1.0 This standard belongs here
#' @noRd
myfunction <- function (...) {
    # ...
}
...
from within the main `R/` directory, the `rrr` package process blocks from
...
While `rrrstats` tags can also be placed in the `src/` directory, the package
currently only parses [`doxygen`](https://github.com/doxygen/doxygen)-style
blocks for code written in C or C++. (Note that `rrr` is currently further
...
be written in `.Rmd` format, for which `@rrrstats` tags must be included within
distinct code chunks. Again, the skeleton package has an example as follows:
```{r readme-ex}
readLines (file.path (d, "README.Rmd"))
```

Those lines illustrate the expected form. `@rrrstats` tags should be within
a single block contained within a dedicated code chunk. `@rrrstats` chunks within
...
### 3.4 `@rrrstatsNA` tags for non-applicable standards

While `@rrrstatsTODO` and `@rrrstats` tags may be placed anywhere within a package,
`@rrrstatsNA` tags used to denote non-applicable standards must be placed within
...
1. Automatically download and insert lists of general and category-specific
   ...
   `R/` directory called `rrr-stats-standards.R`, although any alternative name
   can also be passed to the function (or the file can be renamed after it has
   initially been created).
2. Change your package's `DESCRIPTION` file to use the `rrr` roclets by adding
   `roclets = "rrr::rrr_stats_roclet"`) to the `Roxygen` line, as demonstrated at
...
3. Run [`roxygenise`](https://roxygen2.r-lib.org/reference/roxygenize.html) to
   ...
   a single list of `@rrrstatsTODO` standards.
...
5. Begin addressing the standards by:
    ...
    - Modifying the `@rrrstatsTODO` tag to `@rrrstats`
...
7. Update your documentation as frequently as you like or need, and use the
   ...
   `@rrrstatsTODO` tags into either `@rrrstats` or `@rrrstatsNA` tags.
